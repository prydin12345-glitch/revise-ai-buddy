...
export async function extract(paper: EdexcelBiologyPaper, tier: 'foundation'|'higher', scenario: boolean | ExtractionScenario = false, mode: 'full_mock'|'short_practice' = 'full_mock') {
  const {rows}=edexcelFixture(paper, tier, mode);
  ...
  const snapshot=edexcelSnapshot(paper, tier, mode);
  const exam:any={id:'exam',user_id:'owner',subject_id:'Biology Higher',exam_board:'Edexcel',qualification_level:'GCSE',generation_context:snapshot,
...
export async function boundaryHandler(paper: EdexcelBiologyPaper, name: string, rows: any[], tier: 'foundation'|'higher' = 'foundation', mode: 'full_mock'|'short_practice' = 'full_mock') {
  const writes: Array<{table:string; value:any}>=[];
  const contextSnapshot=edexcelSnapshot(paper, tier, mode);
  const client={auth:{getUser:async()=>({data:{user:{id:'owner'}}})},from(table:string){
    ...
    q.then=(resolve:any,reject:any)=>Promise.resolve({data:table==='exams'?{id:'exam',subject_id:'Biology',exam_board:'Edexcel',qualification_level:'GCSE',generation_context:contextSnapshot}:rows,error:null}).then(resolve,reject);
    return q;
  }};
...
export async function practice(paper: EdexcelBiologyPaper, name:string,tier:'foundation'|'higher',cached=false,invalid=false) {
  const snapshot={...edexcelSnapshot(paper,tier,'short_practice'),profile_id:'profile'};
  const set:any={id:'set',profile_id:'profile',subject_id:'Biology',exam_board:'Edexcel',educational_tier:'GCSE',
    generation_context:snapshot,question_count:2,question_format:'mixed',difficulty_level:'mixed',subtopics:['Key concepts in biology'],notes:''};
  const questions=[{question_number:'1',question_text:'Which cell structure releases energy in aerobic respiration?',question_type:'mcq',marks:1,
    subtopic:'Key concepts in biology',difficulty_level:'easy',choices:{A:'Mitochondrion',B:'Nucleus',C:'Cytoplasm',D:'Ribosome'},expected_answer:'A'},
    {question_number:'2',question_text:invalid?'The investigator measured reaction times.':'Explain how to investigate osmosis fairly.',question_type:'extended',marks:6,
      subtopic:'Key concepts in biology',difficulty_level:'medium',expected_answer:'Repeat measurements.',mark_scheme:fixtureScheme}];
  ...
  const client={from(table:string){
    ...
    q.then=(resolve:any,reject:any)=>{
      ...
      if(table==='question_generation_cache'){reads.push(filters);data=cached?{questions:cachedRows,hit_count:0,subject:'Biology',exam_board:'Edexcel',educational_level:'GCSE'}:null;}
      return Promise.resolve({data,error:null}).then(resolve,reject);
    };return q;
  }};
