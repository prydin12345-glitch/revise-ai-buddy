...
export async function extract(paper: WjecBiologyUnit, tier: 'foundation'|'higher', scenario: boolean | ExtractionScenario = false, mode: 'full_mock'|'short_practice' = 'full_mock') {
  const {rows}=wjecFixture(paper, tier, mode);
  ...
  const snapshot={...wjecSnapshot(paper, tier, mode),...config.snapshotPatch};
  const exam:any={id:'exam',user_id:'owner',subject_id:'Biology Higher',exam_board:'WJEC',qualification_level:'GCSE',generation_context:snapshot,
...
export async function boundaryHandler(paper: WjecBiologyUnit, name: string, rows: any[], tier: 'foundation'|'higher' = 'foundation', mode: 'full_mock'|'short_practice' = 'full_mock') {
  const writes: Array<{table:string; value:any}>=[];
  const contextSnapshot=wjecSnapshot(paper, tier, mode);
  const client={auth:{getUser:async()=>({data:{user:{id:'owner'}}})},from(table:string){
    ...
    q.then=(resolve:any,reject:any)=>Promise.resolve({data:table==='exams'?{id:'exam',subject_id:'Biology',exam_board:'WJEC',qualification_level:'GCSE',generation_context:contextSnapshot}:rows,error:null}).then(resolve,reject);
    return q;
  }};
...
export async function practice(paper: WjecBiologyUnit, name:string,tier:'foundation'|'higher',cached=false,invalid=false) {
  const snapshot={...wjecSnapshot(paper,tier,'short_practice'),profile_id:'profile'};
  const set:any={id:'set',profile_id:'profile',subject_id:'Biology',exam_board:'WJEC',educational_tier:'GCSE',
    generation_context:snapshot,question_count:2,question_format:'mixed',difficulty_level:'mixed',subtopics:[paper==='unit_1'?'1.1 Cells and movement across membranes':'2.1 Classification and biodiversity'],notes:''};
  const questions=[{question_number:'1',question_text:'Which method makes a sample more representative?',question_type:'mcq',marks:1,
    subtopic:paper==='unit_1'?'1.1 Cells and movement across membranes':'2.1 Classification and biodiversity',difficulty_level:'easy',choices:{A:'Random sampling',B:'Largest only',C:'Nearest only',D:'One sample'},expected_answer:'A'},
    {question_number:'2',question_text:invalid?'The investigator measured reaction times.':'Explain how to investigate osmosis fairly.',question_type:'extended',marks:6,
      subtopic:paper==='unit_1'?'1.1 Cells and movement across membranes':'2.1 Classification and biodiversity',difficulty_level:'medium',expected_answer:'Repeat measurements to reduce random error.',mark_scheme:fixtureScheme}];
  ...
  const client={from(table:string){
    ...
    q.then=(resolve:any,reject:any)=>{
      ...
      if(table==='user_preferences')data={curriculum_region:'wales'};
      if(table==='student_cache_slots')data=null;
      if(table==='question_generation_cache'){reads.push(filters);data=cached?{questions:cachedRows,hit_count:0,subject:'Biology',exam_board:'WJEC',educational_level:'GCSE'}:null;}
      return Promise.resolve({data,error:null}).then(resolve,reject);
    };return q;
  }};
