...
describe('offline Biology audit', () => {
  it('reports all thirty-six supported combinations and no invented external scores', () => {
    const rows = auditBiologyTemplates();
    expect(rows).toHaveLength(36);
    expect(new Set(rows.map(r => `${r.packId}/${r.tier}/${r.mode}`)).size).toBe(36);
    ...
    expect(rows.filter(r => r.mode === 'full_mock').map(r => r.marks).sort()).toEqual([100, 100, 100, 100, 100, 100, 100, 100, 90, 90, 90, 90, 90, 90, 80, 80, 80, 80].sort());
  });
