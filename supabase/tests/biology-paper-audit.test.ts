...
describe('offline Biology audit', () => {
  it('reports all twenty supported combinations and no invented external scores', () => {
    const rows = auditBiologyTemplates();
    expect(rows).toHaveLength(20);
    expect(new Set(rows.map(r => `${r.packId}/${r.tier}/${r.mode}`)).size).toBe(20);
    ...
    expect(rows.filter(r => r.mode === 'full_mock').map(r => r.marks).sort()).toEqual([100, 100, 100, 100, 100, 100, 100, 100, 90, 90].sort());
  });
