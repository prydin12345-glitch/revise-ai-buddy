...
import { QuestionTypeSchema, DifficultySchema, PracticeQuestionSchema, GeneratePracticeQuestionsSchema, isAsciiOnly, buildSimplifiedPrompt, sanitizeJsonString, extractToolArgs } from "../_shared/ai-response-parser.ts";
...
async function generateQuestionsInBackground(
  ...
  try {
    ...
    // discarded. A quiz difficulty control is not an input and cannot move the
    // assessment tier.
...
`;
    ...
    const difficultyInstructions = buildDifficultyInstructions(
      ...
      setData.difficulty_mode ?? 'fixed',
      setData.subject_id ?? '',
      setData.educational_tier ?? '',
    );
...
${notesSection}`;
    ...
    // ------------------------------
    // Schemas + JSON recovery toolkit imported from _shared/ai-response-parser.ts
