...
/** Visible error placeholder for broken circuit configs */
const MissingCircuit: React.FC<{ error?: string }> = ({ error }) => (
...
const CircuitDraw: React.FC<CircuitDrawProps> = ({ config, width = 480 }) => {
  try {
    ...
    if (!nodes.length || !wires || wires.length < 3) {
      return <MissingCircuit error={`${nodes.length} nodes, ${wires?.length ?? 0} wires`} />;
    }
    ...
    return <MissingCircuit error={err?.message} />;
  }
};
