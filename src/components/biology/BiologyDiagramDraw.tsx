...
const BiologyDiagramDraw: React.FC<{ config: BiologyDiagramConfig; mode?: 'assessment' | 'solution' }> = ({ config, mode = 'assessment' }) => {
  try {
    ...
    switch (config.type) {
      ...
      case 'punnett_square': return <PunnettSquareDiagram config={config} mode={mode} />;
      case 'food_web':
      case 'food_chain':
