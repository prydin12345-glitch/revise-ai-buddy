  ...
  return (
    <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} width="100%"
      ...
      {Array.from({ length: stages }, (_, i) => (
        <text key={i} x={MARGIN.left + (i + 1) * stageW - stageW / 2 + stageW / 2} y={32}
          textAnchor="middle" fontSize={10} fill="hsl(var(--muted-foreground))">
          {stages === 1 ? 'Trial 1' : stages === 2 ? (i === 0 ? '1st trial' : '2nd trial') : `Trial ${i + 1}`}
        </text>
      ))}
      {showOutcomes && (
        <text x={leafX + 60} y={32} textAnchor="middle" fontSize={10} fill="hsl(var(--muted-foreground))">Outcome</text>
      )}
      {showFinalProbabilities && showOutcomes && (
        <text x={leafX + 150} y={32} textAnchor="middle" fontSize={10} fill="hsl(var(--muted-foreground))">Probability</text>
      )}
        ...
        return (
          <g key={i}>
            ...
            <text x={midX - offsetX} y={midY - offsetY + 4} textAnchor="middle"
              fontSize={11} fontWeight={600} fill="hsl(221 83% 53%)">{edge.label}</text>
            <text x={midX + offsetX} y={midY + offsetY + 4} textAnchor="middle"
