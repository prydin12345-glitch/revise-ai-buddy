...
export const SubjectCard = ({ subject, profileCount }: SubjectCardProps) => {
  ...
  return (
    <button
      ...
      className="group w-full text-left rounded-2xl border border-[hsl(220_6%_20%)] bg-[hsl(220_8%_13%)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/25 hover:border-[hsl(220_6%_26%)] overflow-hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    >
      <div
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10.5px] uppercase tracking-[0.12em] text-muted-foreground/80 mt-1">
        ...
        <div className="mt-5 flex items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {trend && trend !== "neutral" && (
              <span className={`inline-flex items-center gap-1 rounded-full bg-white/[0.04] border border-[hsl(220_6%_20%)] px-2 py-0.5 text-[10.5px] font-medium ${trendColor}`}>
                <TrendIcon className="w-3 h-3" />
                {trend === "up" ? "Improving" : "Needs work"}
              </span>
            )}
            <span className="inline-flex items-center rounded-full bg-white/[0.04] border border-[hsl(220_6%_20%)] px-2 py-0.5 text-[10.5px] font-medium text-muted-foreground tabular-nums">
              {profileCount} {profileCount === 1 ? "profile" : "profiles"}
            </span>
          </div>
        </div>

        <div className="mt-4 h-1 w-full rounded-full bg-white/[0.06] overflow-hidden" aria-hidden="true">
          <div
