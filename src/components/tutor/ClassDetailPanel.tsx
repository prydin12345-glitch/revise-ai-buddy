  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          className="flex h-[100dvh] w-screen max-w-none flex-col overflow-hidden border-border/60 bg-card p-0 shadow-2xl backdrop-blur-sm !left-0 !top-0 !translate-x-0 !translate-y-0 !rounded-none sm:!left-[50%] sm:!top-[50%] sm:h-[min(720px,92vh)] sm:w-[min(980px,96vw)] sm:max-w-[96vw] sm:!translate-x-[-50%] sm:!translate-y-[-50%] sm:rounded-2xl"
          style={{
        ...
        >
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex min-h-0 flex-1 flex-col">
            <div className="border-b border-border/40 bg-card">
              <div className="flex items-start justify-between gap-3 px-4 pb-3 pt-4 sm:items-center sm:px-6 sm:py-4">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <DialogTitle className="pr-1 text-base font-semibold leading-tight tracking-tight break-words sm:text-xl">{groupName}</DialogTitle>
                  <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground sm:gap-2 sm:text-sm">
                    <Badge variant="secondary" className="max-w-full font-normal text-xs break-words">{subjectDisplay}</Badge>
                    {levelDisplay && (
                    ...
                    <span className="text-muted-foreground/60">•</span>
                    <span>{activeAssignmentsCount} active task{activeAssignmentsCount !== 1 ? "s" : ""}</span>
                  </div>
                </div>
                <Button
                  variant="outline"
