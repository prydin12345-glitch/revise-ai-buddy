...
const SUGGESTIONS: Array<{ match: RegExp; gcse: string[]; alevel?: string[] }> = [
  { match: /english.*lit|literature|\beng\b.*lit/i,
...
export function hasTopicSuggestions(subjectName: string): boolean {
  return SUGGESTIONS.some((s) => s.match.test(subjectName || ""));
}
